    ...
    db.query(`SELECT cart_id FROM cart WHERE user_id = ${userId}`, (error, results, fields) => {
      if (error) {
        ...
        if (results.length > 0) {
          ...
          // 새로운 장바구니를 생성하도록 수정
          db.query(`INSERT INTO cart (user_id) VALUES (${userId})`, (error, results, fields) => {
            if (error) {
              reject(error);
            } else {
    ...
    db.query(`SELECT cart_id FROM cart WHERE user_id = ${userId}`, (error, results, fields) => {
      if (error) {
        ...
        if (results.length > 0) {
          ...
          db.query(`DELETE FROM cart_item WHERE cart_id = ${cartId}`, (error, results, fields) => {
            if (error) {
              reject(error);              
            } else {
              // 사용자의 cart_id를 사용하여 (cart) 테이블에서 카트를 삭제
              db.query(`DELETE FROM cart WHERE cart_id = ${userCartId}`, (error, results, fields) => {
                if (error) {
                  if (error.code === 'ER_ROW_IS_REFERENCED_2') {
                    reject(new Error("이미 제거된 장바구니 상품입니다."));
                  } else {
                    reject(error); 
                  }
                } else {
