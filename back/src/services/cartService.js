            ...
            if (results.length > 0) {
              ...
              db.query(
                ...
                (error, itemResults, fields) => {
                  if (error) {
                    reject(
                      new Error("장바구니에서 항목을 삭제하지 못했습니다.")
                    );
                  } else {
                    ...
                    if (itemResults.affectedRows === 0) {
                      ...
                      db.query(
                        ...
                        [cartId],
                        (error, cartResults, fields) => {
                          if (error) {
                            if (error.code === "ER_ROW_IS_REFERENCED_2") {
                              reject(
                                new Error(
                                  "지정한 항목이 이미 장바구니에서 제거되었습니다."
                                )
                              );
                            } else {
                              reject(
                                new Error("장바구니를 삭제하지 못했습니다.")
                              );
                            }
                          } else {
                            resolve(
                              "선택하신 상품이 장바구니에서 제거되었습니다."
                            );
                          }
                        }
                      );
